...
function installDropRateHook() {
  ...
  Interceptor.replace(target, new NativeCallback(
    function (a1, a2, roll, a4) {
      ...
      if (dropCallCount % 500 === 0) {
        bootLog(`[DROP-STATS] #${dropCallCount} | ×${dropRateMultiplier} | ` +
      ...
      // 史诗以上实时通知
      if (rarity >= 4) {
        bootLog(`[DROP-EPIC] roll: ${originalRoll} → ${newRoll} | rarity=${rarity}`)
      }
  ...
  bootLog('[DROP-INIT] CLuckPoint::GetItemRarity hook installed at 0x8550BE4')
}
...
function installDropRate() {
  ...
  installDropRateHook()
  bootLog(`[INFO] 爆率已启用, 倍率: ${dropRateMultiplier}, roll上限保护: 1000000`)
}
